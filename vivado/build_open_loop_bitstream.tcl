...
if {$laplace_mode ne "open_loop"} {error "Set laplace_mode open_loop for this commissioning build."}
gate check open_loop
source [file join $root vivado create_laplace.tcl]
...
set out [file join $root reports laplace_open_loop]
file mkdir $out
...
foreach type {max min} {
    ...
    if {[llength $path]!=1 || [get_property SLACK $path]<0} {
        error "Timing failed ($type), inspect reports/laplace_open_loop"
    }
}
...
file copy -force $bitpath [file join $out laplace_open_loop.bit]
puts "OPEN-LOOP BUILD COMPLETE: reports/laplace_open_loop/laplace_open_loop.bit"
