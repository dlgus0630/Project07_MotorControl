...
open_run impl_1
set out [file join $root reports laplace]
file mkdir $out
...
foreach type {max min} {
    ...
    if {[llength $path]!=1 || [get_property SLACK $path]<0} {error "Timing failed ($type), inspect reports/laplace"}
}
...
file copy -force $bitpath [file join $out laplace.bit]
