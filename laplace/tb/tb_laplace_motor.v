...
module tb_laplace_motor;
    ...
    wire pwm,dir,en;wire [15:0] led;
    integer j,n;
    laplace_basys3_top #(.CLK_HZ(320000),.EXTERNAL_MOTOR(1),.OPEN_LOOP(1)) dut(clk,reset,sw,enc,pwm,dir,en,led);
    initial begin
        ...
        if(n!=8 || en!==1 || dir!==1 || dut.duty!==0)begin
        ...
        if(en!==0 || pwm!==0)begin $display("TEST FAIL open-loop stall cutoff");$finish;end
        sw[0]=0;repeat(12)@(negedge clk);sw[0]=1;repeat(12)@(negedge clk);
        if(en!==0)begin $display("TEST FAIL fault reset required");$finish;end
        reset=1;repeat(5)@(negedge clk);reset=0;repeat(20)@(negedge clk);
        if(en!==0)begin $display("TEST FAIL open-loop reset rearm");$finish;end
        sw[0]=0;repeat(12)@(negedge clk);sw[0]=1;repeat(50)@(negedge clk);
        wait(pwm);#2;reset=1;#1;
        if(pwm!==0 || en!==0)begin $display("TEST FAIL open-loop immediate stop");$finish;end
