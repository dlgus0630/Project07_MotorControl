...
module tb_laplace_top;
    ...
    initial begin
        ...
        if(en!==0)begin $display("TEST FAIL boot with arm high");$finish;end
        sw[0]=0;repeat(20)@(negedge clk);sw[0]=1;repeat(20)@(negedge clk);
        if(en!==1)begin $display("TEST FAIL explicit arm");$finish;end
        wait(external_dut.fault);#1;if(en!==0 || pwm!==0)begin $display("TEST FAIL stall cutoff");$finish;end
        reset=1;repeat(4)@(negedge clk);reset=0;repeat(20)@(negedge clk);
        if(en!==0 || external_dut.fault!==0)begin $display("TEST FAIL reset rearm");$finish;end
        sw[0]=0;repeat(10)@(negedge clk);sw[0]=1;repeat(10)@(negedge clk);
        #2;reset=1;#1;if(en!==0 || pwm!==0)begin $display("TEST FAIL immediate reset cutoff");$finish;end
