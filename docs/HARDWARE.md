...
구동과 C1 사각파 측정까지 완료했다. 초기 open-loop 측정에서 얻은 CPR 771.4는 폐루프 반복 측정과
일치하지 않아 폐기했다. 폐루프 안정 상태에서 C1 172.4 Hz와 출력축 10회전 57.30 s, 추가 5회전
28.70 s를 동시에 측정했으며 합산값은 약 988.4 rising edges/output revolution이다. 감속비와
stall current는 아직 확인되지 않았다.
...
반복 실측 CPR 약 988.4와 110 RPM 기준으로 계산한 값은 18.12이며 RTL 정수 설정값은 18이다.
