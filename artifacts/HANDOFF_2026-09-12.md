...
- Basys3 JA1 20.00 kHz/약 25%/0~3.3 V PWM 실측 PASS
- JA2 약 3.3 V, JA3 약 0 V 및 SW0 OFF 즉시 차단 실측 PASS
...
최신 `artifacts/matlab_input.zip`으로 MATLAB/Simulink를 실행했고 반환된 결과는
`artifacts/matlab_results.zip`에 보관했다. 같은 소스로 전체 XSim 5개 testbench도 통과했다.
향후 RTL, 데이터 또는 동작 모드를 변경하면 MATLAB과 XSim부터 다시 실행한다. gate를 통과시키기
위해 결과 파일이나 해시를 수동으로 만들지 않는다.
...
4. 무부하 25%에서 모터 회전, 전류, C1 pulse/s, 출력축 RPM을 동시에 기록한다.
5. closed-loop로 넘어갈 때 새 `artifacts/matlab_input.zip`으로 MATLAB을 실행하고 전체 XSim을
   다시 통과시킨다.
6. 측정한 `laplace_counts_full_scale`을 넣고 closed-loop를 빌드한다.
7. P부터 조정하고 I를 추가하며 D는 필요한 경우에만 사용한다.
